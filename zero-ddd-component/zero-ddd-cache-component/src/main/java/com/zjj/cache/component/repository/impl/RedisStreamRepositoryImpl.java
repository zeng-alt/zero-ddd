...
public class RedisStreamRepositoryImpl<K, V> implements RedisStreamRepository {
     ...
     * @return 事件列表
     */
    public Map<StreamMessageId, Map<K, V>> readEvents(String streamName, StreamMessageId id, int count, Duration timeout) {
        RStream<K, V> stream = redissonClient.getStream(streamName);
...
//    }
     ...
     * @return 订阅结果
     */
//    public RStreamReadGroupResult subscribeStream(String groupId, String groupStreamName, String consumerName, boolean noAck) {
//        RStream<String, Object> stream = redissonClient.getStream(groupStreamName);
//        return stream.readGroup(groupId, new RStreamReadGroupOptions().consumer(consumerName).noAck(noAck));
//    }
