...
public class ConditionBuilder<T, R> {
    ...
    public ConditionBuilder(QuerydslPredicateExecutor<T> executor, Class<R> domainType) {
        this(executor, TypeInformation.of((Class<T>) domainType),
                domainType, null, null, null, Sort.unsorted(), NO_OP_BINDER_CUSTOMIZER);
    }

    public ConditionBuilder(QuerydslPredicateExecutor<T> executor, TypeInformation<T> domainType, Class<R> resultType,
     ...
     * @return a new {@link ConditionBuilder} instance with all previously
     * configured options and {@code projectionType} applied
     */
    public <P> ConditionBuilder<T, P> projectAs(Class<P> projectionType) {
        Assert.notNull(projectionType, "Projection type must not be null");
        return new ConditionBuilder<>(this.executor, this.domainType, projectionType,
     ...
     * @return a new {@link ConditionBuilder} instance with all previously configured
     * options and {@code Sort} applied
     * @since 1.2.0
     */
    public ConditionBuilder<T, R> cursorStrategy(@Nullable CursorStrategy<ScrollPosition> cursorStrategy) {
        return new ConditionBuilder<>(this.executor, this.domainType, this.resultType,
    ...
    public ConditionBuilder<T, R> defaultScrollSubrange(@Nullable ScrollSubrange defaultSubrange) {
        return new ConditionBuilder<>(this.executor, this.domainType, this.resultType, this.cursorStrategy,
     ...
     * @since 1.2.5
     */
    public ConditionBuilder<T, R> defaultScrollSubrange(
     ...
     * options and {@code Sort} applied
     */
    public ConditionBuilder<T, R> sortBy(Sort sort) {
     ...
     */
    public ConditionBuilder<T, R> customizer(QuerydslBinderCustomizer<? extends EntityPath<T>> customizer) {
    ...
    public DataFetcher<R> single() {
        return new QuerydslConditionDataFetcher.SingleEntityFetcher<>(
    ...
    public DataFetcher<Iterable<R>> many() {
        return new QuerydslConditionDataFetcher.ManyEntityFetcher<>(
    ...
    public DataFetcher<Iterable<R>> scrollable() {
        ...
        return new QuerydslConditionDataFetcher.ScrollableEntityFetcher<>(
